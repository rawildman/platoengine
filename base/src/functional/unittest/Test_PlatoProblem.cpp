...
TEST(PlatoProblem, ParsePlatoProblemEvaluateObjective)
{
    ...
    const pf::Core::ValidatedInput tData{pf::parse_and_validate(tInput)};

    const pf::PlatoProblem tProblem = pf::make_plato_problem(tData);
    const auto tGeometry = pf::GeometryFactory::make_geometry_data(tData.value());
    ...
    // Test Objective
    const auto tObjective = pf::ObjectiveFactory::make_aggregate_objective_function(tData.value().mObjectives);
...
TEST(PlatoProblem, ParseAndValidateInvalidInput)
{
    ...
    EXPECT_THROW(const pf::Core::ValidatedInput tData = pf::parse_and_validate(""), pf::Exception);
}