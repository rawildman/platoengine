...
TEST(Composer, EvaluateComposition)
{
    ...
    const auto tF = Plato::Functional::make_function([](const double aX) { return aX * aX; },
                                                     [](const double aX) { return 2.0 * aX; });
    const auto tG =
        Plato::Functional::make_function([](const double aX) { return 2.0 * aX; }, [](const double) { return 2.0; });
    const auto tFOfG = Plato::Functional::compose(tF, tG);
    EXPECT_EQ(tFOfG.f(1.0), 4.0);
    EXPECT_EQ(tFOfG.f(2.0), 16.0);

    const auto tGOfF = Plato::Functional::compose(tG, tF);
...
TEST(Composer, EvaluateCompositionThreeFunctions)
{
    ...
    const auto tF = pf::make_function([](const double aX) { return std::sqrt(aX); },
                                      [](const double aX) { return 0.5 / std::sqrt(aX); });
    const auto tG =
        pf::make_function([](const double aX) { return std::sin(aX); }, [](const double aX) { return std::cos(aX); });
    const auto tH =
        pf::make_function([](const double aX) { return aX * aX; }, [](const double aX) { return 2.0 * aX; });

    const auto tHOfGOfF = pf::compose(tH, pf::compose(tG, tF));
    constexpr double tExpectedValue = 7.080734182735712e-01;  // From Matlab
    EXPECT_EQ(tHOfGOfF.f(1.0), tExpectedValue);
    constexpr double tExpectedDerivative = 4.546487134128409e-01;  // From Matlab
    EXPECT_EQ(tHOfGOfF.df(1.0), tExpectedDerivative);
}
...
TEST(Composer, TwoDFunctions)
{
    ...
    const auto tF = pf::make_function(pft::TwoDScalarFunction{}, pft::TwoDScalarFunctionGradient{});
    const auto tG = pf::make_function(pft::TwoDVectorFunction{}, pft::TwoDVectorFunctionJacobian{});
    const auto tH = pf::compose(tF, tG);
    {
