 ...
 pugi::xml_document& aDocument)
{
    ...
    auto tSpatialDim = std::find(tValidKeys.mKeys.begin(), tValidKeys.mKeys.end(), aXMLMetaData.mScenario.dimensions());
    if (tSpatialDim == tValidKeys.mKeys.end())
    {
        THROWERR(std::string("Append Problem Description to Plato Analyze Input Deck: Invalid spatial dimensions '")
            + aXMLMetaData.mScenario.dimensions() + "'.  Only three and two dimensional problems are supported in Plato Analyze.")
    }
 ...
 pugi::xml_node& aParentNode)
{
    ...
    auto tLowerPhysics = Plato::tolower(aXMLMetaData.mScenario.physics());
 ...
 pugi::xml_node& aParentNode)
{
    ...
    XMLGen::Private::append_simp_penalty_function(aXMLMetaData.mScenario, tPhysics);
}
