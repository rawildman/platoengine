...
namespace plato::tpetra_integration::unittest
{
...
TEST(TpetraHelpers, PutStdVectorOfCoordOnTPetraMultiVector)
{
    const auto tCommunicator = Tpetra::getDefaultComm();
    // clang-format off
    const std::vector<utilities::Coordinate> tCoordinates = {{1, 11, 21},   
                                                  {2, 12, 22}, 
                                                  {3, 13, 23},
                                                  {4, 14, 24}, 
                                                  {5, 15, 25}, 
    ...
    for (TpetraLocalOrdinal tIndex = 0; tIndex < number_of_local_elements(tContiguousMap); ++tIndex)
    {
        ...
        utilities::test_double_equality_of_components(tCoordinate, tCoordinates[tGlobalInd],
