...
[[nodiscard]] Coordinate element_centroid(const stk::mesh::Entity& aElement, const stk::mesh::BulkData& aBulk);

/// @brief Return a vector of all the element centroids found in bulk data @a aBulk
[[nodiscard]] std::vector<Coordinate> element_centroids(const stk::mesh::BulkData& aBulk);
...
[[nodiscard]] std::vector<Coordinate> element_coordinates(const stk::mesh::Entity& aElement,
...
{
    double operator()(const std::vector<Coordinate>& aCoordinates, VolumeTag) const
    {
        return detail::volume_impl<Topology>(aCoordinates);
    }

    Coordinate operator()(const std::vector<Coordinate>& aCoordinates, CentroidTag) const
    {
    ...
    static auto zero()
    {
        using ReturnType = std::invoke_result_t<ApplyTaggedFunction<Topology>, std::vector<Coordinate>, FunctionTag>;
        return ReturnType{};
    }
};
