...
TEST(ProcessManagerValidation, InValidParsedInputNoProcessManagers)
{
    ...
    EXPECT_FALSE(detail::validate_at_least_one_process_manager(tInput).has_value());
}
...
{
    input_parser::ParsedInput tInput = test_utilities::create_valid_example_input_with_gradient_check();
