...
TEST(ParallelMassAppRegistration, RegisterLoadAndRun)
{
    ...
    auto tConfigurationTempDirectory = integration_tests::utilities::register_test_mass_app(tAppName.mToken, tComm);
    const auto tCriterionName = input_parser::CriterionName{"mass"};
    const auto tValidInput =
        integration_tests::utilities::create_valid_brick_input(tAppName, tCriterionName, tComm.size());
