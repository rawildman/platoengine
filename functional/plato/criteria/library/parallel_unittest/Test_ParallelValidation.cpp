...
TEST(ParallelObjectiveValidation, ValidateMPIRanksVsNumberOfObjectives)
{
    ...
    EXPECT_FALSE(detail::validate_number_of_ranks_vs_objectives({tObjective, tObjective, tObjective}).has_value());

    // Four objectives and three ranks
    EXPECT_FALSE(
        detail::validate_number_of_ranks_vs_objectives({tObjective, tObjective, tObjective, tObjective}).has_value());
}
...
TEST(ParallelObjectiveValidation, ValidateMPIRanksVsNumberOfObjectivesParallelObjectives)
{
    ...
    EXPECT_FALSE(detail::validate_number_of_ranks_vs_objectives({tObjective, tObjectiveOneProcessor}).has_value());

    // Mismatch with two objectives
    tObjectiveOneProcessor.number_of_processors = 2u;
    EXPECT_TRUE(detail::validate_number_of_ranks_vs_objectives({tObjective, tObjectiveOneProcessor}).has_value());
}
}  // namespace plato::criteria::library::unittest
